...
var Recording = /** @class */ (function () {
    function Recording(recording) {
        ...
        this.url = "https:".concat(recording.url);
            ...
            switch (_a.label) {
                case 0:
                    if (name.trim().length < 2) {
                        return [2 /*return*/, Promise.reject('Bird name to search for must have at least two characters')];
                    }
                    client = new rm.RestClient('test');
                    queryName = name.trim().replace(/(\s+)/g, '+').toLowerCase();
                    url = "https://www.xeno-canto.org/api/2/recordings?query=".concat(queryName, "+len:10-25");
                    console.log("Searching for ".concat(queryName));
...
function downloadRecording(targetDir, recording) {
    return __awaiter(this, void 0, void 0, function () {
